...
const FbaAmountModal = ({ user, visible, onClose, onUpdate }) => {
  ...
  const [fbaAmountCom, setFbaAmountCom] = useState("");

  // Initialize values when the modal opens
  useEffect(() => {
    if (user) {
      setFbaAmountCom(user.fbaAmountCom || ""); // Default to empty if not set
    }
  }, [user]);
  ...
  return (
    ...
    >
      <div>
        <label>FBA Amount:</label>
        <Input
          value={fbaAmountCom}
          onChange={(e) => setFbaAmountCom(e.target.value)}
